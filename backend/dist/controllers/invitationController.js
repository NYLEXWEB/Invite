...
exports.getInvitationBySlug = getInvitationBySlug;
...
async function createInvitation(req, res, next) {
    try {
        ...
        while (!isUnique && attempts < 10) {
            slug = (0, slug_1.generateSlug)(8);
            const existing = await Invitation_1.Invitation.findOne({ slug });
            if (!existing) {
                isUnique = true;
            }
            attempts++;
        ...
        const newInvitation = new Invitation_1.Invitation({
            serviceType,
            slug,
            userData,
...
async function getInvitationBySlug(req, res, next) {
    try {
...
async function updateInvitation(req, res, next) {
    try {
        ...
        const { serviceType } = req.body;
        let userData = req.body.userData;
        const invitation = await Invitation_1.Invitation.findById(id);
        if (!invitation) {
...
async function deleteInvitation(req, res, next) {
    try {
        ...
        await Invitation_1.Invitation.findByIdAndDelete(id);
        // Clean up related analytics records
        await Analytics_1.Analytics.deleteMany({ invitationId: invitation._id }).catch(err => console.error('Failed to clean up analytics on deletion:', err));
        res.status(200).json({
            success: true,
